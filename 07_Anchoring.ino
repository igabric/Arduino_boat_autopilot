...
void Anchor_alarm(){
  String text;
  float distanca;
  int tolerance; // allowed distance from the anchor point
  float acc;
  float x_dist;
  float y_dist;
  ...
  int prolaz_vrij = 0;  
  while (digitalRead(PB_1p) == HIGH or digitalRead(PB_10p) == HIGH){ // Anchoring on until the PB_1p and PB_10p buttons are pressed simultaneously
    ...
    distanca = Distance_m(gps_long_anc, gps_lat_anc, gps_long, gps_lat);
