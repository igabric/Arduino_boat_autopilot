...
int select_POI() {
  ...
  while( true ) {
    ...
    // // Serial.print(longArray[count],6);// // Serial.print("  ");// // Serial.print(latArray[count],6);// // Serial.print("  ");// // Serial.println(count);
    lastStateCLK = currentStateCLK;
    int btnState = digitalRead(SW);
    if (btnState == LOW) {
      if (millis() - lastButtonPress > 50) {
...
void rot_encoder_SET_CUSTOM_POI() {
  ...
  //rot_encoder_SET_CUSTOM_COORDINATE(int x_pos, int y_pos, int max_val, int text_size, int min_val = 0, int leading = 2, int custom_value = 0);
