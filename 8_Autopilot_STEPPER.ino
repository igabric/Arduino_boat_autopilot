...
void Autopilot(bool ukljucen){
  ...
  if(ukljucen){
    ...
    if (autopilot_heading){
      ...
      time_count = millis();
      int prolaz_vrij = 0;
//      tft.drawCircle(440, 160, 12, TFT_WHITE);
//      tft.drawCircle(440, 200, 12, TFT_WHITE);
//      tft.fillCircle(440, 160, 10, TFT_YELLOW);
//      tft.fillCircle(440, 200, 10, TFT_GREEN); 
      while (digitalRead(PB_1p) == HIGH or digitalRead(PB_10p) == HIGH){ // Autopilot - heading uključen dok se istodobno ne pritisnu tasteri PB_1p i PB_10p
        prolaz_vrij += 1;
        if(prolaz_vrij == 10){
          temp = temperatura(); 
...
void Navigate_by_POI(){
  ...
  float ttg; int ttg_hours;  int ttg_minutes;  // varijable za određivanje preostalog vremene putovanja
...
//                             UDALJENOSTm (lon1, lati1, lon2, lati2)
//                                   vraća duljinu u metrima

// ************************************************************************************************

float Udaljenost_m(double long1, double lat1, double long2, double lat2){
  char buf1[5];
  double delta = radians(long1-long2);
  double sdlong = sin(delta);
  double cdlong = cos(delta);
...
//                               vrši provjeru valjanosti i ispis kursa
...
void Navigate(){
  ...
  int y_pos = 130;
  String buf1;
  // Usporedba trenutnog i optimalnog kursa te korekcija istog korištenjem autopilota
  // KOREKCIJA KURSA AKO JE ODSTUPANJE VEĆE OD INICIJALNO PODEŠENOG BROJE stupnjeva
