...
// ************************************************************************************************
void Kompas_labels(int polozaj){
  ...
  if (polozaj > 90 and polozaj <= 120){
      label_offset = 0;
      pomak_oznaka = int((polozaj - 90)*faktor);
    }
    else if (polozaj > 120 and polozaj <= 150){
      label_offset = 1;
      pomak_oznaka = int((polozaj - 120)*faktor);
    }
    else if (polozaj > 150 and polozaj <= 180){
      label_offset = 2;
      pomak_oznaka = int((polozaj - 150)*faktor);
    }
    else if (polozaj > 180 and polozaj <= 210){
      label_offset = 3;
      pomak_oznaka = int((polozaj - 180)*faktor);
    }
    else if (polozaj > 210 and polozaj <= 240){
      label_offset = 4;
      pomak_oznaka = int((polozaj - 210)*faktor);
    }
    else if (polozaj > 240 and polozaj <= 270){
      label_offset = 5;
      pomak_oznaka = int((polozaj - 240)*faktor);
    }
    else if (polozaj > 270  and polozaj <= 300 ){
      label_offset = 6;
      pomak_oznaka = int((polozaj - 270)*faktor);
    }
    else if (polozaj > 300  and polozaj <= 330){
      label_offset = 7;
      pomak_oznaka = int((polozaj - 300)*faktor);
    }
    else if (polozaj > 330  and polozaj <= 359){
      label_offset = 8;
      pomak_oznaka = int((polozaj - 330)*faktor);
    }
    else if (polozaj >= 0 and polozaj <= 30){
      label_offset = 9;
      pomak_oznaka = int((polozaj - 0)*faktor);
    }
    else if (polozaj > 30  and polozaj <= 60){
      label_offset = 10;
      pomak_oznaka = int((polozaj - 30)*faktor);
    }
    else {
      label_offset = 11;
      pomak_oznaka = int((polozaj - 60)*faktor);
    }
  ...
  tft.print(Leading_zero(polozaj, 3));
  // tft.drawString(Leading_zero(polozaj, 3), 300, 57, 4);
  // tft.drawNumber(polozaj, 250, 60, 4); 
  tft.drawCircle(300, 57, 3, TFT_YELLOW);
   
  for (int i = 0; i < 8; i++){
    tft.setTextColor(TFT_BLACK);

    // KORIŠTENJE FLASH MEMORIJE ZA POHRANU POLJA STRINGOVA kurs_labels
    // strcpy_P(buf, (char *)pgm_read_word(&(kurs_labels[i])));
...
void SET_HDG(float hdg_set_value){
  ...
  tft.setTextColor(TFT_RED, TFT_BLACK);
  tft.setCursor(65, 220, 4);
  tft.print(gps.cardinal(hdg_set_value)); // ispis strana svijeta slovima
}
// ------------------------------------------------------------------------------------------------
